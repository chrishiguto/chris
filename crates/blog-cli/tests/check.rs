...
fn valid_tree_checks_clean() {
    let posts = blog_cli::check_tree(&fixture_tree("valid"), &blog_cli::manifest()).unwrap();
...
fn invalid_tree_reports_every_problem() {
    let diags = blog_cli::check_tree(&fixture_tree("invalid"), &blog_cli::manifest()).unwrap_err();
...
fn the_repos_real_content_tree_checks_clean() {
    let root: PathBuf = [env!("CARGO_MANIFEST_DIR"), "../../content/blog"]
        .iter()
        .collect();
    let posts = blog_cli::check_tree(&root, &blog_cli::manifest()).unwrap();
