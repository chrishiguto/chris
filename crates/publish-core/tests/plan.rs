...
use publish_core::{check, plan, PostSource};
use registry::{ComponentSpec, Manifest, PropSpec, PropType};
...
fn plan_writes_posts_and_rewrites_index_newest_first() {
    ...
    let doc = content_ast::Document::from_json(&plan.writes[0].value).unwrap();
