use content::{Document, Frontmatter, Node, PropValue, SCHEMA_VERSION};
...
fn old_schema_version_fails_detectably() {
    ...
    match err {
        content::AstError::SchemaVersionMismatch { found, expected } => {
            assert_eq!(found, 0);
...
fn missing_schema_version_fails_detectably() {
    ...
    let err = Document::from_json(&value.to_string()).unwrap_err();
    assert!(matches!(err, content::AstError::Json(_)));
}
...
fn index_entry_matches_kv_index_shape() {
    let entry = content::IndexEntry::new(
        "hello",
        &Frontmatter {
            title: "Hello".into(),
            date: "2026-07-04".into(),
    ...
    let back: Vec<content::IndexEntry> = serde_json::from_value(json).unwrap();
    assert_eq!(back, vec![entry]);
}

#[test]
fn index_entry_defaults_tags_and_draft() {
    let entry: content::IndexEntry = serde_json::from_value(serde_json::json!({
        "slug": "hello",
        "title": "Hello",
        "date": "2026-07-04",
    }))
...
fn description_round_trips_and_stays_absent_when_unset() {
    ...
    let entry = content::IndexEntry::new("hello", &frontmatter);
    let json = serde_json::to_value(&entry).unwrap();
