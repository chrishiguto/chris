...
//!
//! Regenerate goldens with `UPDATE_GOLDEN=1 cargo test -p content-parser`.
...
fn valid_fixtures_match_golden_ast_json() {
    ...
    for path in paths {
        ...
        let doc = content_parser::parse_named(&source, &path.display().to_string())
        ...
        let golden_source = fs::read_to_string(&golden_path).unwrap_or_else(|_| {
            panic!(
                "missing golden {}; run UPDATE_GOLDEN=1 cargo test -p content-parser",
                golden_path.display()
            )
        });
...
#[test]
fn golden_json_round_trips_through_content_ast() {
    for path in fixtures("fixtures/valid") {
        ...
        let doc = content_ast::Document::from_json(&json)
            .unwrap_or_else(|err| panic!("{} does not round-trip: {err}", golden_path.display()));
        assert_eq!(doc.schema_version, content_ast::SCHEMA_VERSION);
    }
}
...
fn invalid_fixtures_all_produce_diagnostics() {
    ...
    for path in paths {
        let source = fs::read_to_string(&path).unwrap();
        let diags = content_parser::parse_named(&source, &path.display().to_string())
