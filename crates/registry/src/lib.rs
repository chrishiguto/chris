...
//! in the site worker, publish validation in the pipeline, and `blog check`.
//!
//! The crate has two layers:
//! - **Manifest** (always available, leptos-free): serde data types describing
//!   the registered vocabulary, consumed by `content-parser` validation.
//! - **Dispatch** (behind the `dispatch` feature): the `#[post_component]`
//!   macro, `inventory`-backed registration, [`lookup`], and [`manifest`].

use serde::{Deserialize, Serialize};

pub use content_ast::PropValue;
