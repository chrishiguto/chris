...
- `disable-erase-components = true` in the leptos workspace metadata: cargo-leptos defaults debug builds to `--cfg erase_components`, but it only builds the client here — worker-build compiles the SSR worker without that cfg, and the erased client expects hydration markers the un-erased worker HTML never renders, so every island panics on hydrate under `just dev`. Release never sets the cfg; only dev breaks.
